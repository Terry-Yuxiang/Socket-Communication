...
int main() {
   ...
   // Start print out.
   std::cout << "ServerA is up and running using UDP on port " << UDPPORT << "." << std::endl;
   sendUsernames();
...
int setUDP() {
   ...
   while (1) {
      ...
      if (header == SEARCH_HEADER) {
         std::cout << "Server A received the usernames from Main Server using UDP over port " << UDPPORT << "."
         << std::endl;
         ...
         std::string withHeader = "serverA;I;" + combineString;
         ...
         std::cout << "Server A finished sending the response to Main Server." << std::endl;
      }
